...
public class MQClientMonitor {
     ...
     * 重置消息队列并发消费者数量
     * @param queueName
     * @param count must greater than zero
     * @return
     */
    public boolean resetQueueConcurrentPrefetchCount(String queueName, int count) {
        Assert.state(count > 0, "参数 'concurrentConsumers' 必须大于0.");
        SimpleMessageListenerContainer container = findContainerByQueueName(queueName);
        if (container.isActive() && container.isRunning()) {
            container.setPrefetchCount(count);
     ...
     * @return
     */
    public boolean resetQueueConcurrentConsumers(String queueName, int concurrentConsumers,int count) {
    ...
    public static final class MessageQueueDatail {
        ...
        public MessageQueueDatail(String queueName, SimpleMessageListenerContainer container) {
            ...
            this.acknowledgeMode = container.getAcknowledgeMode().toString();
